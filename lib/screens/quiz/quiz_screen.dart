...
class _QuizScreenState extends State<QuizScreen> {
  ...
  final Map<String, AudioSource> _preloadedSources = {}; // Cached sources

  int currentQuestionIndex = 0;
  int? selectedOptionIndex;
  bool isAnswerChecked = false;
  bool isQuizCompleted = false;
  bool isPreloading = true; // Flag to show loading progress
  ...
  Future<void> _preloadAllSounds() async {
    ...
    // Preload all question and option sounds
    for (final question in questions) {
      final questionSound = question.correctAnswerSound;
      if (questionSound != null && !_preloadedSources.containsKey(questionSound)) {
        try {
          final source = AudioSource.uri(Uri.parse(questionSound));
          _preloadedSources[questionSound] = source; // Cache sound
        } catch (e) {
          debugPrint("Error preloading question sound: $e");
        }
      }

      for (final option in question.options ?? []) {
        final optionSound = option.sound;
        if (optionSound != null && !_preloadedSources.containsKey(optionSound)) {
          try {
            final source = AudioSource.uri(Uri.parse(optionSound));
            _preloadedSources[optionSound] = source; // Cache sound
          } catch (e) {
            debugPrint("Error preloading option sound: $e");
          }
        }
      }
    }
    ...
    setState(() {
      isPreloading = false; // Preloading finished
    });
  }
  ...
  Future<void> _playSound(String? soundUrl) async {
    ...
    try {
      ...
      // Only play if not already playing the sound
      if (_player.audioSource != source || !_player.playing) {
  ...
  Future<void> _nextQuestion() async {
    ...
    if (currentQuestionIndex < quizProvider.questions.length - 1) {
      ...
      final userStats = await quizProvider.submitSession();
      if (userStats != null) {
        // Handle points, level, streak here
      }
    }
  }
        ...
        child: isQuizCompleted
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                "You scored $currentPoints points",
